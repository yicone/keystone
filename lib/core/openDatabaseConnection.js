...
function addQueryMethods (thinky) {
	...
	const _count = thinky.Query.prototype.count;
	thinky.Query.prototype.count = function (cb) {
		_count.call(this).execute().nodeify(cb);
	};
	thinky.Query.prototype.sort = function (sortString) {
		if (sortString.startsWith('-')) {
			let sortField = sortString.substr(1, sortString.length - 1);
			return this.orderBy(thinky.r.desc(sortField));
		} else {
			return this.orderBy(sortString);
		}
	};
}
...
function addModelMethods (ModelPrototype) {
	// 向 thinky model 添加兼容 mongoogse 的方法
	ModelPrototype.findById = function (id, callback) {
		if (callback) {
			ModelPrototype.get.call(this, id).run().then(function (item) {
				// TODO:
				delete item.list;
				callback(null, item);
			}).catch(function (err) {
		...
		return ModelPrototype.get.call(this, id);
	};
	console.log('before add findOne');
	// 暂不支持字段选择, 否则 thinky 不能执行 run 以返回 document
	ModelPrototype.findOne = function (filter, cb) {
		let query = this.filter(filter);
		query.run().nodeify(function (err, docs) {
			cb(err, docs[0]);
		});
	};
	ModelPrototype.find = function (filter, options) {
		if (!filter) filter = {};
		return this.filter(filter, options);
	};
	ModelPrototype._count = ModelPrototype.count;
	ModelPrototype.count = function (cb) {
		// modelPrototype.__proto__.count.call(this).execute().nodeify(cb);
		ModelPrototype._count.call(this).execute().nodeify(cb);
	};
}
...
module.exports = function openDatabaseConnection (callback) {
	...
	var anyModel = Object.entries(Models)[0][1];
	var anyModelProto = Object.getPrototypeOf(anyModel);
	var ModelProto = Object.getPrototypeOf(anyModelProto);
	addModelMethods(ModelProto);
