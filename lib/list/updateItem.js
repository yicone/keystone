...
function updateItem (item, data, options, callback) {
	...
	if (options.user) {
		var user = _.clone(options.user);
		pruneSchemaVirtuals(user);
		item._req_user = user;
	}
...
function removeMatched (arr, prop, values) {
    var i = arr.length - 1;
	while (i >= 0) {
		var f = arr[i];
		for (var j = 0; j < values.length; j++) {
			if (f[prop].includes(values[j])) {
				arr.splice(i, 1);
				// console.log(arr);
			}
		}
...
function pruneSchemaVirtuals (doc) {
	if (doc.__methods) {
		delete doc.__methods;
	}

	var virtualFields = doc._getModel().virtualFields;
	removeMatched(virtualFields, 'path', ['_', 'list']);
